    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-[#6B6B8A]">Workshops, fests, sports & more</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`flex-shrink-0 px-4 py-1.5 rounded-lg text-xs font-medium capitalize transition-all border ${
              category === cat
          ...
          {loading ? (
            ...
            <div key={event.id} className="glass-card p-4">
              <div className="flex items-start justify-between gap-3 mb-2">
                <h3 className="font-semibold text-sm leading-snug text-[#1A1A2E]">{event.name}</h3>
              ...
              <p className="text-xs text-[#6B6B8A] leading-relaxed mb-3">{event.description}</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={i} className="bg-[#F9F8FF] border border-[#E4E2F0] rounded-lg px-2.5 py-1.5">
                    <p className="text-[10px] text-[#6B6B8A]/70 mb-0.5">{meta.icon}</p>
                    <p className="text-[11px] text-[#1A1A2E] font-medium leading-tight">{meta.label}</p>
                  </div>
                ))}
              </div>
              <p className="text-[10px] text-[#6B6B8A]/70 mt-2">Organized by {event.club}</p>
            </div>
          )) : (
            <div className="text-center py-12 text-[#6B6B8A]">No events found for category "{category}"</div>
          )}
        </div>
        <div>
          <ChatPanel compact />
